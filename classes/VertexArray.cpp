...
void VertexArray::addVertexBufferObject(VertexBuffer &vbo, VertexMemoryLayout &memLayout) {
    vbo.bind();
    this->bind();
    ...
    for(struct GLMemoryLayoutAttribute &attribute:memLayout.getLayoutObject()) {
        ...
        // pointer: suggesting the offsetting value of the attribute you're setting here(such as vertices) in a vertex memory.Because a vertex can contain many attributes such as vertices, normals...
