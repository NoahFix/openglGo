...
int main()
{
    ...
    layout.pushFloat(3);    // Position(x, y, z)
    layout.pushFloat(2);    // Texture(x, y)

    vao.addVertexBufferObject(vbo, layout);

    // Create shaders
    std::string vs;
    readShader("assets/shaders/BasicVertex.vs.glsl", vs);
    std::string fs;
    readShader("assets/shaders/BasicFragment.fs.glsl", fs);
    ...
    Renderer::init(&ibo, &program, &vao, &vbo);
